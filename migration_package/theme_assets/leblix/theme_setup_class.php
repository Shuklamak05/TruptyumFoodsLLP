...
class leblix_Theme_Manager {
	...
	public function start() {
		...
		$theme_actions = array(
			...
			'leblix_page_header_before' => array(
				'method' => 'leblix_page_header_before',
			),
			'leblix_page_header_after'  => array(
				'method' => 'leblix_page_header_after',
			),
			'wp_footer'                => array(
				'method' => 'wp_footer',
			),
		...
		foreach ( apply_filters( 'leblix_theme_actions', $theme_actions ) as $action_key => $action_args ) {
			add_action( $action_key, array(
				$this,
				$action_args['method'],
			), empty( $action_args['priority'] ) ? 10 : $action_args['priority'], empty( $action_args['args'] ) ? 1 : $action_args['args'] );
		}
		foreach ( apply_filters( 'leblix_theme_filters', $theme_filters ) as $filter_key => $filter_args ) {
	...
	public function admin_notices() {
		...
		if($form_links){
            ...
            $message = esc_html__( 'Please set the correct email addresses on your contact forms', 'leblix' );
	...
	public function add_editor_styles() {
		add_editor_style( apply_filters( 'leblix_editor_styles', array(
			'style.editor.css',
		) ) );
	}

	/*
	 * Make leblix available for translation.
	 * Translations can be added to the /languages/ directory.
	 * If you're building a theme based on leblix, use a find and replace
	 * to change 'leblix' to the name of your theme in all the template files.
	 */
	...
	public function default_css_output() {
		leblix_default_css_output( true );
	}
	...
	public function include_theme_files() {
		...
		foreach ( apply_filters( 'leblix_theme_files', $theme_files ) as $theme_file ) {
	...
	public function leblix_page_header_before() {

	}

	public function leblix_page_header_after() {
	...
	public function body_class( $classes ) {
		...
		if ( class_exists( 'leblix_Theme_Manager_custom' ) ) {
			$theme_settings = leblix_Theme_Manager_custom::get_instance()->get_theme_setting( 'customizer' );
		} else {
			$theme_settings = leblix_Theme_Manager::get_instance()->get_theme_setting( 'customizer' );
		}
	...
	public function tiny_mce_before_init( $init_array ) {
		if ( get_theme_mod( 'leblix_site_color', $this->default_color ) ) {
			$init_array['body_class'] = 'leblix_color_' . get_theme_mod( 'leblix_site_color', $this->default_color );
		}
	...
	public function tgmpa_register() {
		...
		if ( ! empty( $plugin_config ) ) {
			$plugin_config = apply_filters( 'leblix_plugin_config', $plugin_config );
			wp_localize_script( 'jquery', 'leblix_plugin_config', $plugin_config );
		}

		$tgmpa_plugins = apply_filters( 'leblix_tgmpa_plugins', $tgmpa_plugins );
	...
	public function customize_register( $wp_customize ) {
		...
		if ( ! empty( $theme_settings['widget-settings'] ) ) {
			...
			$wp_customize->add_section( 'leblix_widget_defaults', array(
				'title'       => esc_html__( 'Widget Styles', 'leblix' ),
				'description' => '',
				'priority'    => 120,
			) );
	...
	public function widget_update_callback( $instance, $new_instance ) {
		...
		if ( ! empty( $theme_settings['widget-settings'] ) ) {
			foreach ( $theme_settings['widget-settings'] as $setting ) {
				if ( ! empty( $setting['id'] ) && isset( $new_instance[ 'leblix_' . $setting['id'] ] ) ) {
					$instance[ 'leblix_' . $setting['id'] ] = $new_instance[ 'leblix_' . $setting['id'] ];
				}
			}
		}
	...
	public function widget_display_callback( $instance, $widget, $args ) {
		...
		if ( ! empty( $theme_settings['widget-settings'] ) ) {
			foreach ( $theme_settings['widget-settings'] as $setting ) {
				...
				if ( empty( $instance[ 'leblix_' . $setting['id'] ] ) ) {
					$instance[ 'leblix_' . $setting['id'] ] = $default;
				}
				if ( ! empty( $setting['id'] ) && isset( $instance[ 'leblix_' . $setting['id'] ] ) ) {
					$my_classname .= esc_attr( 'leblix_'.$setting['id'] . '_'. $instance[ 'leblix_' . $setting['id'] ] );
				}
			}
		}

		$my_classname .= ' leblix_widget_count_'.$widget_count;
				...
				if ( ! empty( $setting['id'] ) && ! empty( $setting['type'] ) ) {
					switch ( $setting['type'] ) {
						case 'select':
							...
							?>
							<div class="leblix_widget_setting">
								<label
									for="<?php echo esc_attr( $widget_instance->get_field_id( 'leblix_' . $setting['id'] ) ); ?>"><?php echo esc_html( $setting['title'] ); ?>
									:</label>
								<select
									name="<?php echo esc_attr( $widget_instance->get_field_name( 'leblix_' . $setting['id'] ) ); ?>"
									id="<?php echo esc_attr( $widget_instance->get_field_id( 'leblix_' . $setting['id'] ) ); ?>"
									class="leblix_widget_style_select">
									<option value=""><?php printf( esc_html__( 'Default (%s)', 'leblix' ), $setting['options'][ $default ] );?></option>
									<?php
									foreach ( $setting['options'] as $key => $val ) { ?>
										<option
											value="<?php echo esc_attr( $key ); ?>" <?php echo ( isset( $settings[ 'leblix_' . $setting['id'] ] ) && $settings[ 'leblix_' . $setting['id'] ] == $key ) ? 'selected' : ''; ?>><?php echo esc_html( $val ); ?></option>
									<?php } ?>
								</select>
							</div>
							<?php
						...
						case 'color':
							...
							<div class="leblix_widget_setting">
								<script type='text/javascript'>
									...
									color_done['<?php echo esc_js( $widget_instance->get_field_id( 'leblix_' . $setting['id'] ) );?>'] = false;
									jQuery(document).ready(function ($) {
										$('.widget-liquid-right #<?php echo esc_js( $widget_instance->get_field_id( 'leblix_' . $setting['id'] ) );?>').wpColorPicker();
								...
								<label
									for="<?php echo esc_attr( $widget_instance->get_field_id( 'leblix_' . $setting['id'] ) ); ?>"><?php echo esc_html( $setting['title'] ); ?>
									:</label>
								<input class="leblix_color_picker" type="text"
								       id="<?php echo esc_attr($widget_instance->get_field_id( 'leblix_' . $setting['id'] )); ?>"
								       name="<?php echo esc_attr($widget_instance->get_field_name( 'leblix_' . $setting['id'] )); ?>"
								       placeholder="<?php esc_attr_e( ' (Press Save)', 'leblix' ); ?>"
								       value="<?php echo esc_attr( ! empty( $settings[ 'leblix_' . $setting['id'] ] ) ? $settings[ 'leblix_' . $setting['id'] ] : '' ); ?>"/>
							</div>
							<?php
							break;
					}
