...
var EnvatoWizard = (function($){
    ...
    function window_loaded(){
        ...
        $('.theme-presets a.cspt-theme-preset-thumbnail').on( 'click', function(e) {
            e.preventDefault();
        ...
        // Set title
        jQuery(document).prop('title', jQuery('#cspt-title').text() ); 
    ...
    function renderMediaUploader() {
        ...
        file_frame.on( 'select', function() {
            ...
            var img = new Image();
            img.onload = function() {
                jQuery('#cspt-swizard-global-color-pallate, #cspt-swizard-secondary-color-pallate').html('');

                jQuery('.cspt-logo-section-right').each(function(){
                ...
                for (var swatch in swatches){
                    if (swatches.hasOwnProperty(swatch) && swatches[swatch]){
                        ...
                        }).appendTo('#cspt-swizard-global-color-pallate, #cspt-swizard-secondary-color-pallate');

                        if( set_globalcolor == false ){
                            jQuery('#cspt-swizard-globalcolor').iris('color', swatches[swatch].getHex() );
                            set_globalcolor = true;
                        }
                        if( set_secondarycolor == false && x==2 ){
                            jQuery('#cspt-swizard-secondarycolor').iris('color', swatches[swatch].getHex() );
                ...
                // now apply click 
                var bgcolor_primary = '';
                jQuery('#cspt-swizard-global-color-pallate > div').on('click', function(e){
                    bgcolor_primary = jQuery(this).css('background-color');
                    jQuery('#cspt-swizard-globalcolor').iris({ 'color': true } );
                    jQuery('#cspt-swizard-globalcolor').iris({ mode: 'rgb' } );
                    jQuery('#cspt-swizard-globalcolor').iris({ 'color': bgcolor_primary } );
                });

                var bgcolor_secondary = '';
                jQuery('#cspt-swizard-secondary-color-pallate > div').on('click', function(e){
                    bgcolor_secondary = jQuery(this).css('background-color');
                    jQuery('#cspt-swizard-secondarycolor').iris({ 'color': true } );
                    jQuery('#cspt-swizard-secondarycolor').iris({ mode: 'rgb' } );
                    jQuery('#cspt-swizard-secondarycolor').iris({ 'color': bgcolor_secondary } );
                });
